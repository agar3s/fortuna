...
void fragment() {
	...
	noise = fbm(coord);
	coord.x += cos(TIME*0.2+noise)*0.01;
	coord.y = coord.y + sin(TIME-coord.y*coord.x)*coord.y*coord.x*0.06*cos(TIME+coord.x-coord.y*1.3);
	//coord.x += coord.y;
	vec4 texturita = texture(SCREEN_TEXTURE, coord);
	texturita.r += noise*texturita.g*sin(TIME*0.3+texturita.b);
	texturita.b += noise*texturita.r*cos(TIME*0.3-texturita.g);
	
	//texturita = texturita +  vec4(vec3(noise)+vec3(-0.6,-1.0,-0.6), 1.0);
	//COLOR = vec4(texturita)+vec4(0.0,0.0,0.0,-1.0);
